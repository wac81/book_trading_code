...
class Account:
    ...
    def __init__(self, money_init, start_date='', end_date='', stop_loss_rate=-0.03, stop_profit_rate=0.05,
        ...
        # self.info = pd.DataFrame(columns=['code', 'name', 'buy_price', 'buy_date', 'buy_num', 'sell_price', 'sell_date',
        #                                   'profit'])
        self.info = pd.DataFrame(columns=['code',  'buy_price', 'buy_date', 'buy_num', 'sell_price', 'sell_date',
                                          'profit'])
    # 股票买入
    ...
    def sell_trigger(self, stock_id, day, all_df, index_df):
        ...
        # print(day, stock_id)
        # 可能会有一些停牌企业，后期再改
        idx = (all_df['date'] == day) & (all_df['code'] == stock_id)
        # print(all_df[idx]['low'])
        if not idx.any(): return False, 3, 0
    ...
    def update(self, day, all_df):
        ...
        for j in range(len(self.stock_id)):
            ...
            idx = (all_df['date'] == day) & (all_df['code'] == self.stock_id[j])
    ...
    def BackTest(self, buy_df, all_df, index_df, buy_price='close'):
        ...
        :return:
        """
        day_info = np.sort(index_df['date'])
        for i in range(len(day_info)):
            day = day_info[i]
            tmp_idx = buy_df['date'] == day
            ...
            if len(tmp_df) != 0:
                for j in range(len(tmp_df)):
                    ...
                    if buy_num == 0:
                        continue
                    buy_num = buy_num * 100
                    self.buy_stock(day, tmp_df['code'][j],
                                   tmp_df['code'][j], tmp_df[buy_price][j], buy_num)
